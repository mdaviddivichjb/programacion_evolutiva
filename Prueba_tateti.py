...
def hay_vacios (tablero):
    for linea in tablero:
        return (linea[0] == 0 or linea[1] == 0 or linea[2] == 0 )
    return False
...
def elijo_rep(poblacion,calificacion_pob,c_reproducciones):
    ...
    for aux in range(0,c_reproducciones):
        numero_azar = randint(0,len(ruleta))
...
# Guardo en un archivo csv la tabla de estados que está en el lugar 0
guardar_datos("C:/tmp/tabla_tateti.csv",poblacion[0])

tabla_estados = cargar_datos("C:/tmp/tabla_tateti.csv")
print(tabla_estados)
